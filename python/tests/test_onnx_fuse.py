...
class OnnxFuseTests(unittest.TestCase):
    def test_resnet18_onnx_fuses_basic_blocks(self) -> None:
        ...
        stride2 = [layer for layer in block_layers if layer.stride == 2]
        self.assertEqual(len(stride2), 3)
        for layer in stride2:
            self.assertLess(layer.in_channels, layer.out_channels)
        self.assertGreater(len(spec.layers), 0)

    def test_convnextv2_atto_backbone_fuses_stage_entry_blocks(self) -> None:
        ...
        onnx_path = Path(tempfile.gettempdir()) / "netkit_convnext_atto_backbone_fuse_test.onnx"
        export_kwargs = dict(
            input_names=["input"],
            output_names=["output"],
            opset_version=17,
        ...
        try:
            torch.onnx.export(FeaturesOnly(backbone), dummy, str(onnx_path), **export_kwargs, dynamo=False)
        except TypeError:
            torch.onnx.export(FeaturesOnly(backbone), dummy, str(onnx_path), **export_kwargs)

        spec = onnx_to_spec(onnx_path, fuse_composite=True)
        block_layers = [layer for layer in spec.layers if layer.kind == "convnextv2_block"]
        self.assertEqual(len(block_layers), 12)
