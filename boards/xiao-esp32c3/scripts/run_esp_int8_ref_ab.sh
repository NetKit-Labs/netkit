#!/usr/bin/env bash
# XIAO ESP32C3 int8 peer A/B — ESP-NN OFF (reference) for netkit + TFLM.
#
# Same MCU methodology as ESP-NN-on A/B: 10 runs × 10 images, discard first
# invoke each run; order swaps nk→tflm and tflm→nk.
#
# Usage:
#   PORT=/dev/cu.usbmodem21101 ./boards/xiao-esp32c3/scripts/run_esp_int8_ref_ab.sh
#   MODELS="cnn" TIMEOUT=900 ./boards/xiao-esp32c3/scripts/run_esp_int8_ref_ab.sh
set -euo pipefail
...
TIMEOUT="${TIMEOUT:-900}"
BAUD="${BAUD:-115200}"
PIO_ENV="${PIO_ENV:-xiao_esp32c3_ref}"
