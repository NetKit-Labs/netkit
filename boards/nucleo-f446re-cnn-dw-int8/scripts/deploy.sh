#!/usr/bin/env bash
# MNIST DS-CNN int8 MCU deploy — discrete steps, no long blocking in one shot.
# Usage:
#   ./scripts/deploy.sh export   # fast quantize from models/mnist_cnn_dw.nk (~seconds)
...
REPO="$(cd "$ROOT/../.." && pwd)"
ELF="$ROOT/build/mnist_cnn_dw_int8_nucleo_f446re.elf"
CAPTURE_SEC="${CAPTURE_SEC:-120}"

step_export() {
  echo "=== [1/4] verify mnist_cnn_dw_int8.nk + int8 test images ==="
  if [[ ! -f "$REPO/models/mnist_cnn_dw_int8.nk" ]]; then
    echo "Missing $REPO/models/mnist_cnn_dw_int8.nk" >&2
    exit 1
  fi
  if [[ ! -f "$REPO/benchmark/tflm/generated/cnn_dw/mnist_cnn_int8_test_images.cc" ]]; then
    echo "Missing cnn_dw int8 test images" >&2
    exit 1
  fi
  ls -la "$REPO/models/mnist_cnn_dw_int8.nk"
  ls -la "$REPO/benchmark/tflm/generated/cnn_dw/mnist_cnn_int8_test_images.cc"
}
