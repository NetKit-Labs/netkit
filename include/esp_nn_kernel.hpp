...
/*
 * ESP-NN float LayerFast surface (mirrors CmsisNnKernel).
 *
 * ESP-NN itself is int8-only; every Try* returns false so ComposedKernel falls
 * back to ReferenceKernel for float32 — same structural slot as CMSIS float
 * LayerFast, without claiming Espressif float kernels that do not exist.
 */
struct EspNnKernel
{
