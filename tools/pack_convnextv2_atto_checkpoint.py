#!/usr/bin/env python3
"""Pack a timm ConvNeXt V2-Atto checkpoint into a .nk file."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Pack timm ConvNeXt V2-Atto into .nk")
    ...
    model = load_backbone_model("convnextv2_atto", num_classes=args.num_classes)
    ...
    assert_packed_matches_reference(arch, weights, torch_forward, seed=42, atol=1e-4, samples=4)
    ...
    write_nk_from_arch(
        ...
        RegressionSuite(
            tolerance=1e-4,
            cases=[RegressionCase(name="ConvNeXt V2-Atto packed checkpoint", input=inp, expected=expected)],
        ),
    )
