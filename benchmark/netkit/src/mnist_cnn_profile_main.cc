// netkit MNIST CNN per-layer invoke profiler (pairs with benchmark/tflm/).

#include "arena.hpp"
#include "arena_util.hpp"
#include "benchmark_cnn_profile.hpp"
...
{
    BenchmarkCnnProfile::TagTickAggregator* aggregator = nullptr;
};
...
int RunBenchmark(const char* model_path)
{
    ...
    BenchmarkCnnProfile::TagTickAggregator profiler;
    ProfileContext profile_ctx{&profiler};

    std::printf("netkit MNIST CNN profile benchmark\n");
    ...
    for (int run = 0; run < kRuns; ++run)
    {
        ...
        for (int i = 0; i < kMnistCnnBenchmarkImageCount; ++i)
        {
            ...
            Tensor& output =
                network->forward_timed(input, arena, LayerTimingCallback, &profile_ctx);
    ...
    BenchmarkStats::PrintSummary("netkit", "cnn", "profile",
                                 BenchmarkStats::Compute(run_averages_us.data(), kRuns));
    profiler.PrintProfileSummaries("netkit", "cnn");
