// TFLM MNIST MLP invoke profiler — benchmark-only translation unit.
...
class TagTickAggregator : public tflite::MicroProfilerInterface {
 public:
  ...
  void PrintProfileSummaries() const {
    ...
    MicroPrintf(
        "PROFILE_SUMMARY runtime=tflm model=mlp kind=meta tag=wall_clock "
        "mean_us=%.3f invoke_count=%d",
        mean_wall, num_invokes_);
    MicroPrintf(
        "PROFILE_SUMMARY runtime=tflm model=mlp kind=meta tag=op_time_sum "
        "mean_us=%.3f invoke_count=%d",
        mean_op_us, num_invokes_);
    MicroPrintf(
        "PROFILE_SUMMARY runtime=tflm model=mlp kind=meta tag=between_op_overhead "
    ...
    for (int i = 0; i < row_count; ++i) {
      MicroPrintf(
          "PROFILE_SUMMARY runtime=tflm model=mlp kind=op tag=%s mean_us=%.3f pct=%.1f "
          "invoke_count=%d",
...
TfLiteStatus RunBenchmark() {
  ...
  if (op_resolver.AddFullyConnected() != kTfLiteOk ||
    ...
    MicroPrintf("Failed to register MLP ops");
    return kTfLiteError;
  }
  ...
  MicroPrintf("TFLM MNIST MLP profile benchmark");
  ...
  BenchmarkStats::PrintSummary("tflm", "mlp", "profile",
